...
int** make_empty_matrix(int row_dim, int col_dim) {
    ...
    @modifies: nothing
    */
   int** array = (int**)malloc(row_dim * sizeof(int*));
   for(int i = 0; i < row_dim; ++i){
    array[i]=(int*)malloc(col_dim * sizeof(int));
   }
    return array;
}
...
int** matrix_multiply(int** matrix_a, int num_rows_a, int num_cols_a,
    ...
    *out_num_cols_c = num_cols_b;
    int** matrix_c = (int**)malloc(num_rows_a * sizeof(int*));
    for(int i = 0; i < num_rows_a; ++i){
        matrix_c[i]=(int*)malloc(num_cols_b * sizeof(int));
    }
